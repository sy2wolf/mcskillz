## Execute point
#revoke the advancementtrigger
advancement revoke @s only mcskillz:melee/meleepoint
#add 1 point
scoreboard players add @s mcskillz.melee_pnt 1
scoreboard players add @s mcskillz.melee_lvl 0
scoreboard players add @s mcskillz.melee_lvlup_pnt 0

execute if score @s mcskillz.melee_pnt >= @s mcskillz.melee_lvlup_pnt run function mcskillz:points/melee_lvlup

execute as @a store result bossbar mcskillz:points value run scoreboard players get @s mcskillz.melee_pnt
execute as @a store result bossbar mcskillz:points max run scoreboard players get @s mcskillz.melee_lvlup_pnt
bossbar set mcskillz:points name {"text":"Melee lvl: ","color":"white","extra":[{"score":{"name":"@s","objective":"mcskillz.melee_lvl"},"color":"light_purple"}]}
scoreboard players set @s mcskillz.bossbarTimer 40
